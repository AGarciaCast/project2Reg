...
Industry <- c('B','D', 'E', 'U', 'C', 'F', 'A')
Service <- c('G','I','S','T', 'N')
Culture <- c('R', 'P')
Social <- c('K', 'M', 'J')
Other <- c('L', 'O', 'X', 'Missing')
...
glmdata$ActivityGroup <- ifelse(glmdata$Activity %in% Industry, "Industry",
                                ifelse(glmdata$Activity %in% Service,"Service",
                                       ifelse(glmdata$Activity %in% Culture,"Culture",
                                              ifelse(glmdata$Activity %in% Social,"Social",
...
glmdata$Travelling.Area.Group <- ifelse(glmdata$Travelling.Area == "Abroad (whole world)", "World",
